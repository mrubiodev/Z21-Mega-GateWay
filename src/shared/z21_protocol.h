...
// desde v0.15 (payload recortado en v0.16, ver historial más abajo — el
// Mega NO debe conocer nunca la password, solo el ESP la guarda). A
// diferencia de NetInfo (que solo se manda una vez resuelta la conexión,
// tras el handshake HELLO/SYNC_ACK), este frame se manda DURANTE
// connectWiFi() en el ESP, según va probando cada red guardada — por eso
// NO depende de que el Mega ya haya completado el handshake de
...
// resultado de la conexión — la password NUNCA sale del ESP por este
// frame; para verla, el portal web del ESP (ver htmlConfigPage() en
// esp8266_wifi.ino) muestra el valor actualmente guardado de cada red.
...
//   - v0.15 (2026-08-09): MEGA_FW_VERSION_MINOR 12->13, ESP_FW_VERSION_MINOR
//     17->18. Nuevo FRAME_TYPE_WIFI_ATTEMPT (ver comentario junto a la
//     constante) y su payload WifiAttempt (ver junto a NetInfo, más
//     arriba): el ESP ahora avisa al Mega en vivo, mientras connectWiFi()
//     va probando cada red guardada, del SSID y la password configurados
//     y del resultado (probando/conectado/fallo/fallback a AP) -- petición
...
//     del monitor serie. connectWiFi() en esp8266_wifi.ino manda un frame
//     por cada red antes de probarla (state=TRYING, con password) y otro
//     al resolverse (CONNECTED/FAILED), más uno final (AP_FALLBACK) si
//     ninguna conecta; mega_z21.ino vuelca cada uno en el log de
//     comunicación de pantalla (handleWifiAttempt(), ver mega_z21.ino) —
//     no se añaden filas nuevas a la cabecera de estado (ya sin hueco, ver
//     historial de display_status_panel.cpp), el log es donde ya vivían
//     el resto de datos de red que no caben ahí (SSID, gateway). Al no
//     depender de "synced", este frame llega igual aunque el handshake
//     HELLO/NET_INFO/SYNC_ACK todavía no se haya completado (de hecho es
//     el caso normal: connectWiFi() ocurre en el setup() del ESP, antes de
//     que el Mega haya podido sincronizar). ACTUALIZAR LOS DOS SKETCHES A
//     LA VEZ.
//   - v0.16 (2026-08-09): MEGA_FW_VERSION_MINOR 13->14, ESP_FW_VERSION_MINOR
//     18->19. CORRECCIÓN sobre v0.15: se retira la password del payload
...
//     versión recortada más arriba); handleWifiAttempt() en mega_z21.ino
//     deja de parsear/mostrar password. sendWifiAttempt() en
//     esp8266_wifi.ino ya no recibe el parámetro pass. Para poder seguir
//     verificando la password guardada (petición de usuario: "en la web
//     si podemos ver la password"), htmlConfigPage() ahora muestra el
//     valor actualmente guardado de cada red junto a su campo, en vez de
//     dejarlo siempre en blanco -- la password sigue viviendo SOLO en la
//     EEPROM del ESP y en el portal web (ya protegido con las credenciales
//     de acceso del propio portal), nunca cruza el enlace serie hacia el
//     Mega. ACTUALIZAR LOS DOS SKETCHES A LA VEZ.
